...
export const updatePostAPI = (id, payload) =>
  axiosInstance.patch(`/posts/${id}`, payload);
export const getMyPostsAPI = () => axiosInstance.get("/posts/mine");
export const deletePostAPI = (id) => axiosInstance.delete(`/posts/${id}`);
export const getMySubscribersAPI = () =>
  axiosInstance.get("/subscriptions/subscribers");
export const getAuthorProfileAPI = (id) => axiosInstance.get(`/users/${id}`);
export const getAuthorPostsAPI = (id, page = 1) =>
  axiosInstance.get(`/users/${id}/posts?page=${page}`);
...
export const searchPostsAPI = (q, page = 1) =>
  axiosInstance.get(`/search?q=${encodeURIComponent(q)}&page=${page}`);
