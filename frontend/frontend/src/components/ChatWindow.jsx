...
const ChatWindow = ({ conversation }) => {
  ...
  const handleSend = () => {
    ...
    socket.emit('message:send', { conversationId: conversation.id, content: input }, (res) => {
  ...
  if (!conversation) {
    return <div className="chat-empty-state">Select a conversation to start chatting</div>;
  }

  return (
    <div className="chat-window">
      <div className="chat-header">
        <img src={conversation.other_user_avatar} alt="" className="chat-header-avatar" />
      ...
      <div className="chat-messages">
        {loading ? (
          ...
          <>
            {messages.map((m) => (
              <div key={m.id} className={`chat-bubble-row ${m.sender_id === user.id ? 'own' : ''}`}>
